...
Nous ne voulons pas effectuer l'appel API dans ce test en particulier, nous voulons simplement vérifier que le message est affiché. Dans ce cas, nous pourrions utiliser les `stubs`, qui apparaissent dans l'option de `mount` : `global`.
...
::: tip
Pour substituer, vous pouvez utiliser la clé dans `components` ou le nom de votre composant. Si les deux sont donnés dans `global.stubs`, la clé sera utilisée en premier.
:::

## Substituer tous les Composants Enfants

Parfois, vous pourriez vouloir substituer *tous* les composants personnalisés. Par exemple, vous pourriez avoir un composant comme ceci :
...
Imaginez que chacun des `<Complex>` fasse quelque chose de compliqué, et que vous soyez seulement intéressé par le test de rendu du bon message de `<h1>`. Vous pourriez faire quelque chose comme ceci :
...
Parfois, les directives effectuent des opérations assez complexes, comme réaliser beaucoup de manipulation de DOM ce qui peut entraîner des erreurs dans vos tests (en raison de JSDOM ne ressemblant pas au comportement entier du DOM). Un exemple courant sont les directives de tooltip de différentes bibliothèques, qui dépendent généralement fortement de la mesure de la position/taille des nœuds DOM.

Dans cet exemple, nous avons un autre `<App>` qui rend un message dans une infobulle.
...
Nous ne voulons pas que le code de la directive `Tooltip` soit exécuté dans ce test, nous voulons simplement vérifier que le message est affiché. Dans ce cas, nous pourrions utiliser les `stubs`, qui apparaissent dans l'option de `mount` : `global` en passant `vTooltip`.
...
test('substitue le composant avec un template personnalisé', () => {
  ...
  console.log(wrapper.html());
  // <h1 class="with-tooltip">Bienvenue dans Vue.js 3</h1>
...
::: warning
Le remplacement des directives ne fonctionnera pas sur les composants fonctionnels ou `<script setup>` en raison de l'absence de nom de directive à l'intérieur de la fonction [withDirectives](https://vuejs.org/api/render-function.html#withdirectives). Considérez le mocking du module de directive via votre cadre de test si vous avez besoin de remplacer une directive utilisée dans un composant fonctionnel. Consultez https://github.com/vuejs/core/issues/6887 pour un projet permettant de débloquer cette fonctionnalité.
:::

## Slots Par Défaut et `shallow`

Comme `shallow` remplace tout le contenu d'un composant, aucun `<slot>` ne sera rendu lors de l'utilisation de `shallow`. Bien que ce ne soit pas un problème dans la plupart des cas, il existe des scénarios où cela n'est pas idéal.
...
Si vous utilisez `shallow`, le slot ne sera pas affiché, car la fonction d'affichage dans `<custom-button />` est remplacée. Cela signifie que vous ne pourrez pas vérifier que le texte correct est affiché !
...
Comme ce comportement est global, et non basé sur un `mount` par `mount`, vous devez vous souvenir de l'activer et le désactiver avant et après chaque test.
...
En règle générale, **plus vos tests ressemblent à la manière dont votre logiciel est utilisé**, plus vous pouvez avoir confiance en eux.

Les tests qui utilisent `mount` rendront toute la hiérarchie de composants, ce qui est plus proche de ce que l'utilisateur vivra dans un vrai navigateur.
...
Gardez à l'esprit ceci : que vous effectuiez un montage complet ou un rendu partiel, les bons tests se concentrent sur les entrées (`props` et interaction de l'utilisateur, comme avec `trigger`) et les sorties (les éléments DOM qui sont rendus et les événements), et pas sur les détails d'implémentation.
