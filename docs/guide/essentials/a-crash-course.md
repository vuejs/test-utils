...
This test is similar to the previous two; we find an element and interact with it in the same way (we use `setValue` again, since we are interacting with an `<input>`).
